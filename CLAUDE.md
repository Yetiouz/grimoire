...
- `/app` — the Vite + React + TypeScript + Tailwind v4 scaffold (Milestone 1 foundation). The design system exists: a UI kit (`app/src/components/ui/` — Panel, Button, Badge, StatTile, LogEntryRow) and a living style-guide page (`app/src/pages/style-guide/`) that `App.tsx` renders directly — no router yet, nothing else to route to. `pnpm verify` (audit + typecheck + lint + test + build) is wired up locally and in `.github/workflows/verify.yml`.
- **Pirata One font file not committed yet.** `app/src/index.css` expects a self-hosted `app/public/fonts/PirataOne-Regular.woff2` (the display face, per SPEC's locked design decisions), but every font CDN and mirror (Google Fonts, jsDelivr, cdnjs, GitHub, apt) was blocked by network egress settings in the environment that wired this up — the actual font binary couldn't be fetched from anywhere reachable. Until that file is added, `--font-brand` falls back to Cinzel/serif (the Tokens section's type specimen calls this out visually so the fallback isn't silent). Get it from https://fonts.google.com/specimen/Pirata+One or https://gwfh.mranftl.com/fonts/pirata-one (woff2, weight 400) and save it at that exact path.
...
- Match the landing page's design language: dark theme, CSS variables in `:root` (see `index.html` — `--bg`, `--panel`, `--purple`, etc.), Inter for UI text, Pirata One for brand headers.

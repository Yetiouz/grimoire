...
- `/app` — the Vite + React + TypeScript + Tailwind v4 scaffold (Milestone 1 foundation). The design system exists: a closed set of 8 named typography levels (`app/src/lib/typography.ts` — display/h1/h2/h3/body/bodySecondary/label/numeric, sized via `--text-*` theme tokens in `index.css`), a UI kit (`app/src/components/ui/` — Panel, Button, Badge, StatTile, LogEntryRow), and a living style-guide page (`app/src/pages/style-guide/`) that `App.tsx` renders directly — no router yet, nothing else to route to. Pirata One is self-hosted at `app/public/fonts/PirataOne-Regular.woff2` (converted from the official Google Fonts TTF; `OFL.txt` sits alongside it). `pnpm verify` (audit + typecheck + lint + test + build) is wired up locally and in `.github/workflows/verify.yml`.

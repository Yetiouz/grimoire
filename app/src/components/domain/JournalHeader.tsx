 ...
 * mark is sized to visually span both lines and then some (`h-10` as
 * of a settling-down pass through h-9 -> h-11 -> h-10, on top of a
 * same-day crop of the source file itself -- logo.webp used to carry
 * a lot of dead transparent canvas around the glyph, so the same
 * height class kept reading smaller than intended; once that got
 * fixed the glyph filled its box enough that h-11 read a "tad" too
 * big and came back down) rather than the earlier hero-row's
 * single-line height — a real, deliberate size change from that
 * original single-line mark, not the "same size everywhere, no
 * responsive drift" guarantee the previous round's doc comment
 * described (that guarantee still holds: nothing here carries a
 ...
 * lines (a deliberate NEGATIVE margin on the session-meta line, now
 * `-mt-1` -- text.label's own line-height leading was still reading as
 * visible space even once the margin hit Tailwind's smallest positive
 * step, and stayed visible through a first, smaller negative step too)
 * were both hand-tuned against the live render rather than picked up
 * front -- worth knowing before nudging either again, since "more/less
 * space" here is reading the actual rendered gap, not a formula. logo.webp itself also carries hand-tuned padding now, not
    ...
    <header className="border-b border-line">
      <div className="flex items-center justify-between gap-4 px-4 py-2">
        <button
          ...
          className="-ml-2 flex min-h-11 min-w-0 items-center gap-2 rounded-button py-1.5 pl-2 pr-3 hover:bg-panel2"
        >
          <img src="/logo.webp" alt="" className="h-10 w-auto shrink-0" />
          <div className="min-w-0 text-left">
            <h1 className={cx(text.h3, 'truncate')}>{campaignName}</h1>
            <p className={cx(text.label, '-mt-1 truncate')}>{sessionMeta}</p>
          </div>
        </button>

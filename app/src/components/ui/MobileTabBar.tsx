...
function TabButton({ view, icon, label, active, onSelect }: { view: MobileView; icon: IconName; label: string; active: boolean; onSelect: (view: MobileView) => void }) {
  return (
    <button
      ...
      className={cx(
        'flex min-h-11 min-w-14 flex-col items-center justify-end gap-1 py-1',
        text.label,
...
export function MobileTabBar({ active, onSelect, onOpenDice, diceDisabled, className }: MobileTabBarProps) {
  return (
    <div
      className={cx(
        'tabbar-safe-bottom flex shrink-0 items-end justify-around border-t border-line bg-panel/95 px-2 pt-1',
        className,
      )}
    >
      {TABS.map((tab) => (

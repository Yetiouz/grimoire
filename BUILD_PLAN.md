...
**Suggested order for what's left**, not a hard requirement: item 14's realtime/presence gap (the one with a real milestone riding on it) is closed as of item 16, 2026-08-14 — a two-account playtest is now the way to confirm it end-to-end, not blocked on more building. Items 7 and 8's remaining halves are each small, self-contained, and are probably the next plan gate. Item 13 (Encounter mode) is the biggest remaining lift and was always sequenced last for a reason — worth keeping it there rather than pulling it forward.
